...
TEST_CASE("ExpressionParser: parseExpression - expressions") {
    ...
    REQUIRE_THROWS_WITH(parser.parseExpression(), "invalid expression: factor must appear before ) and operator must appear after )");
    ...
    REQUIRE_THROWS_WITH(parser.parseExpression(), "invalid expression: factor must appear before )");
}
